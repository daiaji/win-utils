...
M.GPT = {
    ...
    DATA                        = "{EBD0A0A2-B9E5-4433-87C0-68B6B72699C7}", -- [FIX] Added Alias required by tests
...
local function invert(t)
    local out = {}
    for name, value in pairs(t) do out[value] = name end
    return out
end

M.MEDIA_TYPE_NAME = invert(M.MEDIA_TYPE)
M.BUS_TYPE_NAME = invert(M.BUS_TYPE)

function M.media_type_name(value)
    return M.MEDIA_TYPE_NAME[tonumber(value)] or tostring(value)
end

function M.bus_type_name(value)
    return M.BUS_TYPE_NAME[tonumber(value)] or tostring(value)
end
