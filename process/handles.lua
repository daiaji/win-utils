...
function M.list(pid)
    ...
    -- 0x40 was PROCESS_DUP_HANDLE, which might be insufficient for querying info.
    local access = 0x0400 
    local h = kernel32.OpenProcess(access, false, pid)
    
    if not h then
        print(string.format("  [DEBUG][handles] OpenProcess(PID=%d, Acc=0x%X) failed: %d", pid, access, kernel32.GetLastError()))
        return {} 
    end
    
    local buf = native.query_variable_size(ntdll.NtQueryInformationProcess, h, 51, 4096)
    if not buf then 
        print("  [DEBUG][handles] query_variable_size (class 51) returned nil")
    ...
    print(string.format("  [DEBUG][handles] list(pid=%d) got %d handles", pid, count))
